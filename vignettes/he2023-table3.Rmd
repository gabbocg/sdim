...
Science*, 69(9). The table compares four ways of summarising a large
set of candidate factor proxies down to a few risk factors that price
the 48 Fama-French value-weighted industry portfolios. Performance is
measured by the total adjusted $R^2$ (%) of the pricing regressions.
...
- **PLS**: extract the $K$ components of $G$ that are most predictive
  of the test-asset returns.
- **RRA**: the reduced-rank approach of He et al. (2023). It searches
  for $K$ linear combinations of the proxies in $G$ whose loadings best
  explain the returns of the basis assets. Formally, the loading matrix
  in the regression of returns on proxies is restricted to rank $K$,
  and that restriction is what shrinks $L \approx 70$ proxies down to a
  handful of usable factors.

The motivation is empirical: many candidate factors have been proposed
in the literature, but most carry little incremental pricing
information once one accounts for the others. RRA is designed to find
the small linear subspace that retains the pricing-relevant content.
...
- the **PCA**, **PLS**, and **RRA** rows fit the corresponding
